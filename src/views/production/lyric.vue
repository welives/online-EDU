<template>
  <div class="app-container">
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      ...
      >
        <el-form-item label="词标题" prop="title">
          <el-input v-model="temp.title" />
        </el-form-item>
        <!-- <el-form-item label="词类型" prop="type">
          <el-select v-model="temp.type" placeholder="请选择">
            <el-option v-for="(item, index) in typeOptions" :key="index" :label="item" :value="item" />
          </el-select>
        </el-form-item> -->
        <el-form-item label="词正文" prop="content">
          <tinymce ref="content" v-model="temp.content" :height="300" :width="750" />
        </el-form-item>
      </el-form>
...
<script>
import { lyricList, createLyric, updateLyric, deleteLyric } from '@/api/production'
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        // type: [{ required: true, message: '请选择词类型', trigger: 'blur' }],
        content: [{ required: true, message: '请填写词', trigger: 'blur' }]
      }
    }
  },
  computed: {
  ...
  methods: {
    ...
    getList() {
      ...
      lyricList(this.listQuery).then((response) => {
    ...
    handleUpdate(row) {
      ...
      this.dialogFormVisible = true
      this.$nextTick(() => {
        this.$refs['content'].setContent(this.temp.content)
        this.$refs['dataForm'].clearValidate()
      })
    },
    // 编辑
